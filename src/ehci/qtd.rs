...
impl Drop for TransferGuard<'_> {
    fn drop(&mut self) {
        ...
        // Use DWT cycle counter for timeout (1 second max wait)
        let start = cortex_m::peripheral::DWT::cycle_count();
        let timeout_cycles = 600_000_000; // 1 second @ 600MHz

        while self.qtd.is_active() {
            // Check timeout to prevent infinite loop if hardware hangs
            if cortex_m::peripheral::DWT::cycle_count().wrapping_sub(start) > timeout_cycles {
                #[cfg(feature = "defmt")]
                defmt::error!("Transfer timeout in TransferGuard drop - hardware may have hung");
                break; // Force break to prevent deadlock
            }
            core::hint::spin_loop();
        }
    }
