    ...
    OC: InputPin,
{
    ...
    fn handle_overcurrent_fault(&mut self) -> Result<()> {
        ...
        self.last_fault_time.store(
            (Self::get_system_time_us() / 1000) as u16,
            Ordering::Relaxed,
        );
    ...
    OC: InputPin,
{
    ...
    fn reset_overcurrent(&mut self) -> Result<()> {
        match self.state {
            ...
            VbusState::PermanentFault => self.manual_recovery(),
            _ => Ok(()),
        }
    }
...
pub mod rtic {
    ...
    impl<V: VbusPowerControl> RticVbusControl<V> {
        pub fn new(inner: V) -> Self {
            ...
            Self { inner, last_state }
        }

        /// Enable VBUS with event notification
        pub fn enable(&mut self) -> Result<VbusEvent> {
            match self.inner.enable_vbus() {
                Ok(()) => Ok(VbusEvent::EnableRequested),
                Err(UsbError::PortError) => Ok(VbusEvent::OverCurrentDetected),
                Err(e) => Err(e),
            }
        }
        ...
        pub fn update(&mut self) -> Result<Option<VbusEvent>> {
            ...
            let event = if current_state != self.last_state {
                match (self.last_state, current_state) {
                    ...
                    (VbusState::OverCurrentFault, VbusState::FaultRecovery) => {
                        Some(VbusEvent::RecoveryStarted)
                    }
                    (VbusState::FaultRecovery, VbusState::PowerGood) => {
                        Some(VbusEvent::RecoverySuccessful)
                    }
                    (VbusState::FaultRecovery, VbusState::OverCurrentFault) => {
                        Some(VbusEvent::RecoveryFailed)
                    }
