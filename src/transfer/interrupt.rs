...
impl InterruptTransfer {
    ...
    pub fn schedule<const N_QH: usize, const N_QTD: usize>(
        ...
        if !matches!(self.state(), InterruptState::Idle | InterruptState::Complete) {
    ...
    pub fn process_completion<const N_QH: usize, const N_QTD: usize>(
        ...
        if status & (1 << 0) != 0 { // NAK bit (assuming bit 0 for NAK)
        ...
        if status & 0x7C != 0 { // Error bits (excluding STALL and NAK)
        ...
        self.bytes_transferred.store(bytes_received, Ordering::Release);
    ...
    pub fn is_failed(&self) -> bool {
        matches!(self.state(), InterruptState::Failed | InterruptState::Stalled)
    }
