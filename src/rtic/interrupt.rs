...
use crate::ehci::{PortSc, UsbSts};
use crate::error::{Result, UsbError};
use crate::perf::{InterruptTimer, PerfCounters};
use crate::rtic::{DeferredEvent, UrgentEvent, UsbInterruptHandler};
...
impl UsbInterruptContext {
    ...
    pub fn process_deferred(&self) -> Result<()> {
        ...
        if status & (1 << 3) != 0 {
            ...
            self.handler
                .deferred_events
                .enqueue(DeferredEvent::FrameRollover);
        }
        ...
        if status & (1 << 5) != 0 {
            ...
            self.handler
                .deferred_events
                .enqueue(DeferredEvent::AsyncAdvance);
        }
    ...
    fn handle_usb_error(&self, status: u32) -> Result<()> {
        ...
        if status & (1 << 4) != 0 {
            ...
            self.handler
                .urgent_events
                .enqueue(UrgentEvent::HostSystemError);
        }
    ...
    fn queue_port_change(&self) -> Result<()> {
        ...
        for port in 0..8 {
            if self.port_has_change(port) {
                self.handler
                    .deferred_events
                    .enqueue(DeferredEvent::PortChange { port });
            }
        }
        Ok(())
    }
...
pub mod app {
    ...
    pub mod usb_host_app {
        ...
        async fn enumerate_device(ctx: enumerate_device::Context, port: u8) {
            let mut transfer_manager = ctx.shared.transfer_manager;
            let mut descriptor_allocator = ctx.shared.descriptor_allocator;
